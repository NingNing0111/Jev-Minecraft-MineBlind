...
以下截图来自 <http://127.0.0.1:3010/> 的真实运行页面，使用 Chrome 在 1600 × 1000 视口中截取；局部图直接截取页面面板或弹窗，未注入演示数据。三张图分别展示世界视角、环境与资源、单次决策详情，避免重复相同的全屏布局。

### 01 · 看世界，也看当前目标

完整 HUD 将第一人称世界渲染与自主控制状态放在同一屏：顶部是生命、食物和决策延迟，底部叠加只读装备 / 快捷栏，右侧是当前战术、宏观目标路线及会话指标。

![MineBlind 实时世界视角、装备快捷栏与宏观规划总览](docs/images/hud-overview.png)

**这是生存调试现场，不是通关展示。** 截图时生命值仅为 1/20、饱食度为 0/20，界面显示食物优先的生存目标。规划文字表达的是模型意图，不代表已经找到食物或完成恢复。

### 02 · 放大查看环境与资源

状态面板特写展示生命与饱食度、威胁雷达、空间距离，以及附近矿物和背包物品。资源条目带有距离、坐标、最低工具要求与掉落说明，可用于核对下一步采集条件。感知范围仅限当前已加载区域；「可采集」也不保证路径和实际执行一定成功。

<p align="center">
  <img src="docs/images/hud-resources.png" width="420" alt="MineBlind 状态面板特写：威胁、空间感知、附近矿物和背包资源" />
</p>

### 03 · 打开一次决策，看输入与输出

在「决策日志」中点击「查看输入 / 输出」，即可打开单次决策详情。弹窗按需加载当时的上下文、技能选择结果，以及可用的 Jev 请求正文、原始响应和错误 / 降级原因。图中收起了较长的输入区，聚焦输出；展开输入即可核对决策当时的观测，而不是用最新状态猜测。

![MineBlind 单次决策详情弹窗：输入上下文、技能选择输出与请求响应记录](docs/images/hud-decision-detail.png)

技能选择输出不等于执行成功；本地降级记录没有模型响应。暂停 / 继续与游戏声音开关位于「控制台」。图片保存在 [`docs/images/`](docs/images/)，可打开查看原图。
...
同时运行 Minecraft 服务器和机器人，见 [DOCKER.md](DOCKER.md)。提供原生架构 Dockerfile、健康检查、持久卷和本机绑定端口；Minecraft EULA 必须由你自行接受。已验证 AMD64 镜像构建与离线测试；此前亦验证了本机 ARM64 Docker 部署、Minecraft 连接及 HUD / Viewer 实时显示。完整自主行为、长期稳定性及通关能力仍待验证。
